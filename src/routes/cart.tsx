  ...
  if (detailed.length === 0) {
    return (
      <div className="mx-auto max-w-2xl px-4 py-24 text-center">
        ...
        <Link
          to="/shop"
          className="mt-6 inline-block rounded-xl gradient-brand px-5 py-3 font-semibold text-white"
        >
          Start shopping
        </Link>
      </div>
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-[1fr_380px]">
        <div className="space-y-3">
          {detailed.map(({ product, qty }) => (
            <div
              key={product.id}
              className="flex flex-wrap items-center gap-4 rounded-2xl glass p-4"
            >
              ...
              <div className="flex-1 min-w-0">
                <Link
                  ...
                  className="line-clamp-1 font-semibold hover:text-brand-accent"
                >
                  {product.name}
                </Link>
                <div className="mt-1 text-xs text-muted-foreground">
                  {product.region} · {product.duration}
                </div>
              </div>
              <div className="inline-flex items-center rounded-xl border border-white/10 bg-white/5">
                <button onClick={() => setQty(product.id, qty - 1)} className="p-2">
              ...
              <button
                ...
                className="rounded-lg p-2 text-muted-foreground hover:text-rose-400"
                aria-label="Remove"
              >
        ...
        <div className="h-fit rounded-2xl glass p-6">
          ...
          <Link
            to="/checkout"
            className="mt-6 flex items-center justify-center rounded-xl gradient-brand px-4 py-3.5 text-sm font-semibold text-white shadow-lg"
          >
